...
newtype BytesResponse = BytesResponse{bytesResponsePayload ::
                                      Hs.ByteString}
                        deriving (Hs.Show, Hs.Eq, Hs.Ord, Hs.Generic, Hs.NFData)
 
instance HsProtobuf.Named BytesResponse where
        nameOf _ = (Hs.fromString "BytesResponse")
 
instance HsProtobuf.HasDefault BytesResponse
 
instance HsProtobuf.Message BytesResponse where
        encodeMessage _
          BytesResponse{bytesResponsePayload = bytesResponsePayload}
          = (Hs.mconcat
               [(HsProtobuf.encodeMessageField (HsProtobuf.FieldNumber 1)
                   bytesResponsePayload)])
        decodeMessage _
          = (Hs.pure BytesResponse) <*>
...
instance HsJSONPB.ToJSONPB BytesResponse where
        toJSONPB (BytesResponse f1) = (HsJSONPB.object ["payload" .= f1])
        toEncodingPB (BytesResponse f1)
          = (HsJSONPB.pairs ["payload" .= f1])
 
instance HsJSONPB.FromJSONPB BytesResponse where
        parseJSONPB
          = (HsJSONPB.withObject "BytesResponse"
               (\ obj -> (Hs.pure BytesResponse) <*> obj .: "payload"))
 
instance HsJSONPB.ToJSON BytesResponse where
        toJSON = HsJSONPB.toAesonValue
        toEncoding = HsJSONPB.toAesonEncoding
 
instance HsJSONPB.FromJSON BytesResponse where
        parseJSON = HsJSONPB.parseJSONPB
 
instance HsJSONPB.ToSchema BytesResponse where
        declareNamedSchema _
          = do let declare_payload = HsJSONPB.declareSchemaRef
               bytesResponsePayload <- declare_payload Proxy.Proxy
               let _ = Hs.pure BytesResponse <*> HsJSONPB.asProxy declare_payload
               Hs.return
                 (HsJSONPB.NamedSchema{HsJSONPB._namedSchemaName =
                                         Hs.Just "BytesResponse",
